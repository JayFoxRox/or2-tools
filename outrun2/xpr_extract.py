...
def xpr_extract(f):
  ...
  os.makedirs("/tmp/or2/%s/xpr/" % (filename), exist_ok=True)

  header = data[0:xpr_offset]
  open("/tmp/or2/%s/pre-xpr-header.bin" % (filename), 'wb').write(header)
  ...
  while offset < header_size:
    ...
    if False:
      ...
      if export_full:
        open("/tmp/or2/%s/xpr/%d-texture-0-0-full.bin" % (filename, i), 'wb').write(pixel_data)
      ...
      if export_trim:
        open("/tmp/or2/%s/xpr/%d-texture-0-0-trim.bin" % (filename, i), 'wb').write(pixel_data)
